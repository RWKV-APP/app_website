...
function translateToEnglish(chineseContent: ReleaseNoteContent, version: string): string {
  ...
  for (const update of chineseContent.updates) {
    ...
    translated = translated.replace(/优化了图像问答的速度表现/g, 'Optimized image Q&A performance');
    translated = translated.replace(/新增古今回答模式/g, 'Added ancient-modern answer mode');
    translated = translated.replace(/添加了全新的 1\.5B 翻译模型/g, 'Added new 1.5B translation model');
    translated = translated.replace(/现在我们可以设置应用程序的字体了/g, 'Now you can set the application font');
    translated = translated.replace(/续写模式适配黑暗模式/g, 'Continuation mode adapted to dark mode');
    translated = translated.replace(/高通 NPU 加速新增 Snapdragon 8 \+ Gen 1 支持/g, 'Qualcomm NPU acceleration: Added Snapdragon 8+ Gen 1 support');
    translated = translated.replace(/提升了应用程序在高通骁龙芯片上的表现（模型需要重新下载）/g, 'Improved app performance on Qualcomm Snapdragon chips (models need to be re-downloaded)');
    translated = translated.replace(/调整了内部分用户界面的展示效果/g, 'Adjusted internal UI display effects');
    translated = translated.replace(/进行了若干性能优化/g, 'Performed several performance optimizations');
    translated = translated.replace(/修复可一个导致 batch 推理时 logits 互相污染的 bug/g, 'Fixed a bug causing logits cross-contamination during batch inference');
  ...
  let linksSection = chineseContent.linksSection
    ...
    .replace(/App 下载链接维护服务器与前端项目：/g, 'App download link maintenance server and frontend project: ');
